	...
	const std::string &aBaseName, const std::string &aDir)
{
	...
	H5::H5File file = H5::H5File(h5_file, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);

	//Write time data
	TimeSeriesTable states_table = _model->getComponent<StatesTrajectoryReporter>("states_reporter").getStates().exportToTable(*_model);

	std::vector<double> time = states_table.getIndependentColumn();
	...
	H5::DataSet dataset = file.createDataSet("/time", time_datatype, time_dataspace);

	dataset.write(&time[0], time_datatype);

	//Write Contact Data
	std::string cnt_group_name{ "/WISCO_ElasticFoundationForce" };
	file.createGroup(cnt_group_name);

	for (int i = 0; i < _contact_force_names.size(); ++i) {

		std::string contact_name = _contact_force_names[i];
		file.createGroup(cnt_group_name + "/" + contact_name);

		//Write Mesh Contact Data
		h5WriteMeshContactData(file, contact_name, cnt_group_name);
	}

	file.close();
}

void WISCO_ContactAnalysis::h5WriteMeshContactData(H5::H5File& file, 
	const SimTK::String& contact_name, const SimTK::String& cnt_group_name){

	//Collect Raw Contact Data
	std::vector<SimTK::Matrix> mesh1FaceData, mesh1PointData;
	std::vector<std::string> mesh1FaceDataNames, mesh1PointDataNames;
	...
	//Write Face Data
	int nFaceDataField = mesh1FaceData.size();

	for (int j = 0; j < nFaceDataField; ++j) {
		std::string mesh1_data_path{ cnt_group_name + "/" + contact_name + "/" + mesh1FaceDataNames[j] };
		h5WriteMatrixData(file, mesh1FaceData[j], mesh1_data_path);
		
		std::string mesh2_data_path{ cnt_group_name + "/" + contact_name + "/" + mesh2FaceDataNames[j] };
		h5WriteMatrixData(file, mesh2FaceData[j], mesh2_data_path);
	}

	//Write Vertex Data
	int nPointDataField = mesh1PointData.size();

	for (int j = 0; j < nFaceDataField; ++j) {
		std::string mesh1_data_path{ cnt_group_name + "/" + contact_name + "/" + mesh1PointDataNames[j] };
		h5WriteMatrixData(file, mesh1PointData[j], mesh1_data_path);

		std::string mesh2_data_path{ cnt_group_name + "/" + contact_name + "/" + mesh2PointDataNames[j] };
		h5WriteMatrixData(file, mesh2PointData[j], mesh2_data_path);
	}

	//Collect Summary Data
	std::vector<SimTK::Vector> mesh1DoubleData;
	std::vector<SimTK::Matrix_<SimTK::Vec3>> mesh1Vec3Data;
	std::vector<std::string> mesh1DoubleNames, mesh1Vec3Names;

	std::vector<SimTK::Vector> mesh2DoubleData;
	std::vector<SimTK::Matrix_<SimTK::Vec3>> mesh2Vec3Data;
	std::vector<std::string> mesh2DoubleNames, mesh2Vec3Names;

	collectMeshContactSummary(contact_name,
		mesh1DoubleData, mesh1DoubleNames, mesh1Vec3Data, mesh1Vec3Names,
		mesh2DoubleData, mesh2DoubleNames, mesh2Vec3Data, mesh2Vec3Names);

	//Write Summary Data
	int nDoubleField = mesh1DoubleData.size();

	for (int j = 0; j < nDoubleField; ++j) {
		std::string mesh1_data_path{ cnt_group_name + "/" + contact_name + "/" + mesh1DoubleNames[j] };
		h5WriteVectorData(file, mesh1DoubleData[j], mesh1_data_path);

		std::string mesh2_data_path{ cnt_group_name + "/" + contact_name + "/" + mesh2DoubleNames[j] };
		h5WriteVectorData(file, mesh2DoubleData[j], mesh2_data_path);
	}

	int nVec3Field = mesh1Vec3Data.size();
	/*
	for (int j = 0; j < nDoubleField; ++j) {
		std::string mesh1_data_path{ cnt_group_name + "/" + contact_name + "/" + mesh1Vec3Names[j] };
		h5WriteMatrixData(file, mesh1Vec3Data[j], mesh1_data_path);

		std::string mesh2_data_path{ cnt_group_name + "/" + contact_name + "/" + mesh2Vec3Names[j] };
		h5WriteMatrixData(file, mesh2Vec3Data[j], mesh2_data_path);
	}*/
}

void WISCO_ContactAnalysis::h5WriteMatrixData(H5::H5File& file, 
	SimTK::Matrix& data_matrix, const SimTK::String& data_path) {
	...
	H5::DataSet dataset = file.createDataSet(data_path, datatype, dataspace);
...
void WISCO_ContactAnalysis::h5WriteVectorData(H5::H5File& file,
	SimTK::Vector& data_vector, const SimTK::String& data_path) {
