    ...
    )
{
    ...
    updConnector<WISCO_ContactMesh>("mesh1").connect(mesh1);
    updConnector<WISCO_ContactMesh>("mesh2").connect(mesh2);
}
...
void WISCO_ElasticFoundationForce::extendAddToSystem(SimTK::MultibodySystem& system) const
{
    ...
    int mesh1_nTri = getConnector<WISCO_ContactMesh>("mesh1").getConnectee().getNumFaces();
    int mesh2_nTri = getConnector<WISCO_ContactMesh>("mesh2").getConnectee().getNumFaces();
...
{
    std::cout << "Active Triangles: " << getCacheVariableValue<int>(state,"mesh2_n_active_tri") << std::endl;
    //std::cout << "Active Triangles: " << mesh2_nActiveTri << "/" << mm2.getNumFaces() << std::endl;
}
...
double WISCO_ElasticFoundationForce::collision_detection(const SimTK::State& state, SimTK::Vector_<SimTK::SpatialVec>& bodyForces) const
{
    ...
    SimTK::Vector_<Vec3> tri_cen1 = mm1.getTriangleCenters();
    SimTK::Vector_<Vec3> tri_cen2 = mm2.getTriangleCenters();

    SimTK::Vector_<Vec3> tri_nor1 = mm1.getTriangleNormals();
    SimTK::Vector_<Vec3> tri_nor2 = mm2.getTriangleNormals();
    ...
    //Transfrom mesh2 (child) centers and normals to ground for ray casting
    const SimTK::Vector_<SimTK::Vec3>& mesh1_ver0 = mm1.getVertex0();
    const SimTK::Vector_<SimTK::Vec3>& mesh1_ver1 = mm1.getVertex1();
    const SimTK::Vector_<SimTK::Vec3>& mesh1_ver2 = mm1.getVertex2();

    SimTK::Vector_<Vec3> tri_cen1_ground(mm1.getNumFaces());
    SimTK::Vector_<Vec3> mesh1_ver0_ground(mm1.getNumFaces());
    SimTK::Vector_<Vec3> mesh1_ver1_ground(mm1.getNumFaces());
    SimTK::Vector_<Vec3> mesh1_ver2_ground(mm1.getNumFaces());

    for (int i = 0; i < mm1.getNumFaces(); ++i) {
        tri_cen1_ground(i) = GtoM1.shiftFrameStationToBase(tri_cen1(i));
        mesh1_ver0_ground(i) = GtoM1.shiftFrameStationToBase(mesh1_ver0(i));
        mesh1_ver1_ground(i) = GtoM1.shiftFrameStationToBase(mesh1_ver1(i));
        mesh1_ver2_ground(i) = GtoM1.shiftFrameStationToBase(mesh1_ver2(i));

    }

    SimTK::Vector_<Vec3> tri_cen2_ground(mm2.getNumFaces());
    SimTK::Vector_<Vec3> tri_nor2_ground(mm2.getNumFaces());

    for (int i = 0; i < mm2.getNumFaces(); ++i) {
        tri_cen2_ground(i) = GtoM2.shiftFrameStationToBase(tri_cen2(i));
        tri_nor2_ground(i) = GtoM2.xformFrameVecToBase(tri_nor2(i));
    }
    ...
    for (int i = 0; i < mm2.getNumFaces(); ++i) {
        ...
        if (mesh2_target_tri(i) >= 0) {
            ...
            //Check if still in contact with same triangle as previous time step
            double depth = 0.0;
            Vec3 contact_point(0);

            Vec3 vertex0 = mesh1_ver0_ground(mesh2_target_tri(i));
            Vec3 vertex1 = mesh1_ver1_ground(mesh2_target_tri(i));
            Vec3 vertex2 = mesh1_ver2_ground(mesh2_target_tri(i));

            if (rayIntersectTriTest(center, -normal, vertex0, vertex1, vertex2, contact_point, depth))
            {
                if (min_proximity < depth && depth < max_proximity) {
                    ...
                    //applyTriPressureForce(state, bodyForces, mesh2_tri_pressure(i), tri_area2(i), tri_cen2(i), -normal, center_in_m1);
                    mesh2_nActiveTri++;
                    same++;
                    continue;
                }
            ...
            for (int j = 0; j < nNeighborTri; ++j) {
                ...
                vertex0 = mesh1_ver0_ground(neighborTri(j));
                vertex1 = mesh1_ver1_ground(neighborTri(j));
                vertex2 = mesh1_ver2_ground(neighborTri(j));

                if (rayIntersectTriTest(center, -normal, vertex0, vertex1, vertex2, &contact_point, &depth)) {
                    if (min_thickness < depth && depth < max_thickness) {

                        depth2[i] = depth;
                        mesh2TriPressure(i) = computeTriPressure(depth);
                        applyTriPressureForce(state, bodyForces, mesh2TriPressure(i), tri_area2(i), tri_cen2(i), -normal, center_in_m1);
        ...
        if (m1->rayCollision(origin, vector, true, min_proximity, max_proximity)) {
            ...
            mesh2_tri_proximity(i) = computeTriProximity(m1, tri_cen2_ground(i));
    ...
    //markCacheVariableValid(state, "mesh1_tri_pressure");
    //markCacheVariableValid(state, "mesh2_tri_pressure");
    //markCacheVariableValid(state, "mesh1_tri_proximity");
    //markCacheVariableValid(state, "mesh2_tri_proximity");
    /*
    auto val = updCacheVariableValue<SimTK::Vector>(state, "mesh2_tri_pressure");
    for (int i = 0; i < mm2.getNumFaces(); i++) {
        if (mesh2_tri_pressure(i) > 0.0) {
            std::cout << mesh2_tri_pressure(i) << std::endl << " " << val(i) << std::endl;
            break;
        }
    }
        */
    ...
    for (int i = 0; i < mm2.getNumFaces(); i++) {
        if (mesh2_target_tri(i) > 0) {
            std::cout << p2(i) << " " << mesh2_target_tri(i) << std::endl;
            break;
        }
    }
    */
...
double WISCO_ElasticFoundationForce::computeTriProximity(CollisionModel3D* coldet_model,SimTK::Vec3 tri_cen_ground) const
{

    //Contact point on mesh1
    float cnt_point[3];
    coldet_model->getCollisionPoint(cnt_point, false);
    
    SimTK::Vec3 cen_cnt_point;

    for (int j = 0; j < 3; j++) {
        cen_cnt_point(j) = cnt_point[j] - tri_cen_ground(j);
    }
