...
// Default constructor.
WISCO_IdealMuscle::WISCO_IdealMuscle()
{
    setNull();
    constructProperties();
}
...
WISCO_IdealMuscle::WISCO_IdealMuscle(const std::string& aName,
	double aMaxIsometricForce) : WISCO_IdealMuscle()
{
	setName(aName);
	setMaxIsometricForce(aMaxIsometricForce);
	set_use_force_length_velocity_properties(false);
	set_applies_passive_force(false);
	set_ignore_pennation(true);
...
//_____________________________________________________________________________
// Set the data members of this WISCO_IdealMuscle to their null values.
void WISCO_IdealMuscle::setNull()
{
...
void WISCO_IdealMuscle::constructProperties()
{
	constructProperty_use_force_length_velocity_properties(false);
	constructProperty_applies_passive_force(false);
	constructProperty_ignore_pennation(true);
...
void WISCO_IdealMuscle::calcMusclePotentialEnergyInfo(const SimTK::State& s,
...
void WISCO_IdealMuscle::calcFiberVelocityInfo(const State& s, FiberVelocityInfo& fvi) const
{
...
calcMuscleDynamicsInfo(const State& s, MuscleDynamicsInfo& mdi) const
{
	...
	double normActiveForce;
	if (get_use_force_length_velocity_properties()) {
		normActiveForce = mdi.activation
			* mli.fiberActiveForceLengthMultiplier
			* fvi.fiberForceVelocityMultiplier;
	}
	else {
 ...
 */
double WISCO_IdealMuscle::computeActuation(const State& s) const
{
    const MuscleLengthInfo& mli = getMuscleLengthInfo(s);
    
	double force;
	if (get_ignore_pennation()) {
		force = getFiberForce(s);
	}
	else {
		force = getFiberForce(s)*mli.cosPennationAngle;
	}
...
double WISCO_IdealMuscle::computeIsometricForce(State& s, double activation) const
{
	double isoForce;
	...
	if (get_use_force_length_velocity_properties()) {
		...
		isoForce = (activation*activeForceLength + passiveForceLength)
			* cos(getPennationAngle(s));
	}
	else {
		isoForce = activation*get_max_isometric_force();
	}
